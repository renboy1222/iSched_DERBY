...
public class DayDAOImpl extends DBConnection implements DayDAO {
    ...
    public void addDay(Day day) {
        try {
            getDBConn();
            java.sql.PreparedStatement ps = getCon().prepareStatement("INSERT INTO ALDRIN.DAY (ID,DAY) VALUES  (?,?) ");
    ...
    public void updateDay(Day day) {
        try {
            getDBConn();
            java.sql.PreparedStatement ps = getCon().prepareStatement("UPDATE ALDRIN.DAY SET DAY =?  WHERE DAY.ID = ?");
    ...
    public void deleteDay(Day day) {
        try {
            getDBConn();
            java.sql.PreparedStatement ps = getCon().prepareStatement("UPDATE ALDRIN.DAY SET DELETED =? WHERE DAY.ID = ? ");
    ...
    public ArrayList<Day> selectDay() {
        ...
        try {
            String query = "SELECT * FROM ALDRIN.DAY WHERE DAY.DELETED =FALSE  ORDER BY DAY ASC ";
            getDBConn();
...
//            closeConnection();
    ...
    public Boolean isDuplicate(String name) {
        ...
        try {
            getDBConn();
            PreparedStatement statement = getCon().prepareStatement("SELECT   DAY.ID FROM    ALDRIN.DAY WHERE DAY ='" + name + "'");
            ResultSet rs = statement.executeQuery();
            while (rs.next()) {
                Long idl = rs.getLong("ID");
                if (idl > 0) {
